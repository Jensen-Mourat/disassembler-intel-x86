...
export class _Disassembler {
  ...
  private processOperand(
    ...
    if (op1.includes('imm')) {
      if (op1 === 'imm32') {
        return { operand1: { value: this.nextByteXtimes(4, next) } };
      }
      if (options?.is16Bit) {
        return { operand1: { value: this.nextByteXtimes(2, next) } };
      }
      return { operand1: { value: next() } };
    }
    return { operand1: { register: operand1 } };
  }

  private nextByteXtimes(x: number, next: Function) {
    let result = '';
    for (let i = 0; i < x; i++) {
      result += next();
    }
